...
struct GameView: View {
    ...
    var body: some View {
        ...
        .onChange(of: engine.isWon) { _, newValue in if newValue { showResult = true } }
        .onChange(of: engine.isLost) { _, newValue in if newValue { showResult = true } }
        .onAppear {
    ...
    private func restart() {
        let eng = GameEngine(level: level)
        scene = GameScene(engine: eng)
        scene.onLemmingTapped = { id in eng.applySelectedSkill(to: id) }
        isPaused = false
        showResult = false
    }
}
...
private struct HUDTopBar: View {
    ...
    var body: some View {
        HStack(spacing: 16) {
            ...
            statBadge(loc.string(.hudLemmingsSaved), "\(engine.savedCount)/\(engine.level.neededToSave)")
